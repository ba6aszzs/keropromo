...
export default function Categories({ c2 }: { c2: string }) {
  return (
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((cat) => (
            <div key={cat.title} className="border px-6 py-8 text-center transition-all sm:px-8 sm:py-10" style={{ borderColor: "var(--line)" }}>
              <h3 className="font-display text-base font-black sm:text-lg">{cat.title}</h3>
              <p className="mt-2 text-xs leading-relaxed sm:text-sm" style={{ color: "var(--muted)" }}>{cat.desc}</p>
              <span className="mt-4 inline-block border px-3 py-1.5 text-[9px] font-bold tracking-[0.12em] uppercase rounded-md" style={{ borderColor: `${c2}20`, color: c2 }}>{cat.tag}</span>
            </div>
          ))}
        </div>
      </div>
